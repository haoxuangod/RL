...
from RL.common.utils.decorator.Meta import SerializeMeta, SubclassTrackerMeta, Serializer,load_all_modules
from RL.RLBases import ExperienceReplayBuffer, ReplayBuffer, RLAgent, \
    StateProcessor, RLInnerModelBase, OffPolicyInnerModel
from RL.common.replaybuffer import InnerBuffer


class HRLNodeMeta(SerializeMeta,SubclassTrackerMeta):
    pass

class HRLNodeBases(metaclass=HRLNodeMeta):
    '''
    不维护一个model可用于已知策略的节点
    '''
    ...
    def parent(self, value):
        if not isinstance(value, HRLNodeBases) and not value is None:
            raise ValueError("parent must be an instance of HRLNodeBases or None but got {}".format(type(value)))
        self._parent = value
    ...
    def subtasks(self, value):
        if not isinstance(value, (tuple, list)):
            raise TypeError("subtasks must be a tuple or list")

        for subtask in value:
            if not isinstance(subtask, HRLNodeBases):
                raise TypeError(f"subtask:{subtask} must be an instance of HRLNodeBases")
    ...
    def onEpisodeFinished(self, episode, total_reward):
        pass
...
class HRLStrategyNode(HRLNodeBases):
    '''
    用于自定义策略的节点
    '''
    def __init__(self, name, state_processor=None,parent=None):
...
class HRLNode(HRLNodeBases):
    ...
    def parent(self,value):
        if not isinstance(value,HRLNodeBases) and not value is None:
            raise ValueError("parent must be HRLNodeBases or None but got {}".format(type(value)))
        self._parent = value
        if self._parent is not None:
            self.model.parent_model=self._parent.model
        else:
            self.model.parent_model=None
    @property
    def subtasks(self):
        return self._subtasks
    @subtasks.setter
    ...
    def onEpisodeStart(self,episode):
        self.model.onEpisodeStart(episode)
    def onStepFinished(self,episode,state, next_state, action, reward, done, info):
        self.model.onStepFinished(episode,state,next_state,action,reward,done,info)
    def onEpisodeFinished(self,episode,total_reward):
        self.model.onEpisodeFinished(episode,total_reward)
    def append_experience(self,data,episode):
...
class HRLInnerModel(OffPolicyInnerModel):
    '''
    整颗树抽象成一个模型(和整体设计保持一致)
    '''

    def __init__(self,state_space=None,action_space=None,memory_capacity=10000,memory=None,gamma=0.99,state_processor=None,tree=None,batch_size=32):
        super().__init__(state_space=state_space,action_space=action_space,gamma=gamma,batch_size=batch_size,
                         state_processor=state_processor,exploration_strategy=None,memory_capacity=memory_capacity,memory=memory)
        self.tree = tree
        self.gamma=gamma
        #记录每一步要更新的节点
    ...
    def check_available(self):
        def check(cur):
            if not len(cur.subtasks)==0:
                if cur.model.action_space.__class__ not in [int, gymnasium.spaces.Discrete]:
                    raise ValueError(f"The model in Node {cur} is not a primitive task so action space must be Discrete or int but got {cur.model.action_space}")
            else:
                if not cur.get_action_space()==self.action_space:
                    raise ValueError(f"The model in Node {cur} is a primitive task so action space must be equal to the model:action_space:{self.action_space}  ")
            for child in cur.subtasks:
                check(child)
        check(self.tree.root)
    def load_best_model(self):
        def load(node):
            if hasattr(node,"load_best_model"):
                node.load_best_model()
            for child in node.subtasks:
                load(child)
        load(self.tree.root)
    ...
    def _select_action(self,state):
        while not self.tree.currentNode.is_primitive():
            idx=self.tree.currentNode.select_action(self.tree.currentNode.process_state(state))
            self.tree.currentNode.last_select_action=idx
            #print(f"current:{self.tree.currentNode} action:{idx}")
            self.tree.currentNode.l_visit_num=self.steps_done+1
            self.tree.currentNode = self.tree.currentNode.subtasks[idx]
        if self.tree.currentNode.l_visit_num==-1 or self.tree.currentNode.r_visit_num==-1:
            self.tree.currentNode.l_visit_num=self.steps_done+1
            self.tree.currentNode.r_visit_num=self.steps_done+1
        else:
            self.tree.currentNode.r_visit_num=self.steps_done+1

        return self.tree.currentNode.select_action(self.tree.currentNode.process_state(state))
    def _onEpisodeStart(self,episode):
        def dfs(node):
            self.tree.currentNode.onEpisodeStart(episode)
    ...
    def _onStepFinished(self,episode,state,next_state,action,reward,done,info):
        ...
        while done or self.tree.currentNode.is_terminal(next_state):
            ...
            action1=action
            if not self.tree.currentNode.is_primitive():
                action1=self.tree.currentNode.last_select_action
            #assert state1==state
            self._store_hierarchical_exp(self.tree.currentNode,episode,state1,action1,reward1,next_state,done,len1)
            self.tree.currentNode.l_visit_num = self.tree.currentNode.r_visit_num = -1
            self.tree.currentNode.onStepFinished(episode,self.tree.currentNode.process_state(state),
                                                 self.tree.currentNode.process_state(next_state), action, reward, done, info)
    ...
    def _onEpisodeFinished(self,episode,total_reward):
        ...
        def dfs(node):
            self.tree.currentNode.onEpisodeFinished(episode,total_reward)
            for subtask in node.subtasks:
                dfs(subtask)
        dfs(self.tree.root)
